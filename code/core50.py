...
class Core50:
    ...
    @staticmethod
    def load_single_png_image(base_dir: str, fname: str, image_shape:[int]) -> tf.Tensor:
        '''
        ...
        # Guarantee the image shape
        image = tf.ensure_shape(image, image_shape)

        return image

    @staticmethod
    def prepare_single_example(base_dir: str, example: tf.Tensor, image_shape:[int]) -> [tf.Tensor]:
        '''
        ...
        :param image_shape: expected shape of the image
        ...
        # Load image
        img = Core50.load_single_png_image(base_dir, fname, image_shape)
    ...
    def create_dataset(self,
                       ...
                       shuffle: int =0,
                       py_func: bool=True,
                       image_shape:[int]=[128, 128, 3])->tf.data.Dataset:
        '''
        ...
        if py_func:
            # WARNING: something has changed in the current version of tensorflow
            # The dataset does not properly infer its size in this case
            ds = ds.map(lambda x: tf.py_function(func=Core50.prepare_single_example, inp=[base_dir, x, image_shape],
                                                 Tout=(tf.float32, tf.int8)),
                        num_parallel_calls=num_parallel_calls)
        else:
            ds = ds.map(lambda x: Core50.prepare_single_example(base_dir, x, image_shape),
                        num_parallel_calls=num_parallel_calls)
